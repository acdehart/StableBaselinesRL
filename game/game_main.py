...
def main():
    ...
    while True:
        lap += 1
        agent = paddle_boy()
        trial = ball_game(agent, show=False)
...
def calculate_score(agent):
    ...
    dist_bias = - abs((agent.ball_distance / 400) ** 2)
    print(f"H {hits} | RT {round(run_time, 1)}s | DB {round(dist_bias, 1)} | Score: {round(score, 3)} ")
